...
role_crud = BaseCRUD[RoleModelDB,RoleCreate,RoleUpdate](RoleModelDB, RoleCreate, RoleUpdate)
...
async def update_role_by_id(role_id: uuid.UUID, role: RoleUpdate, db: CurrentAsyncSession, 
                    current_user: UserModelDB = Depends(current_active_user)):
    # checking the current user as super user
    if not current_user.is_superuser:
        raise HTTPException(status_code=400, detail="Not Authorized to create roles")
...
async def delete_role_by_id(role_id: uuid.UUID, db: CurrentAsyncSession, 
                    current_user: UserModelDB = Depends(current_active_user)):
    # checking the current user as super user
    if not current_user.is_superuser:
        raise HTTPException(status_code=400, detail="Not Authorized to create roles")
