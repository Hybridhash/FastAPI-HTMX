...
async def create_db_and_tables():
    ...
    async with async_session_maker() as session:
        ...
        user = User(email="superuser@admin.com", hashed_password= password_helper.hash("password123"), 
                        is_superuser=True, is_active=True)
        if not query.scalars().first():
            logger.info(f"Creating superuser... email: {user.email} | password: password123")
            session.add(user)
            await session.commit()
        else:  # pragma: no cover
            logger.info(f"Superuser already exists.... email: {user.email} | password: password123")
